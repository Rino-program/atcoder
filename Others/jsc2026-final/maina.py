...
def main() -> None:
    # ここに解答を書く
    # ランレングス圧縮する？
    # なんか似たような問題あったよねABC-Dだっけ？
    # 出来ない条件は、max(length) > sum(length) - max(length) + 1 で判定できる
    T = INT()
    for i in range(T):
        S = STR()
        Sr = run_length_encode(S)
        # 文字を数字にする
        Sr = [(ord(c) - ord('a'), count) for c, count in Sr]
        ma = max(count for _, count in Sr)
        su = sum(count for _, count in Sr)
        mi = min([count for _, count in Sr if count > 1] or [0])
        if ma > su - ma + 1:
            print(-1)
            continue
        print(mi)
