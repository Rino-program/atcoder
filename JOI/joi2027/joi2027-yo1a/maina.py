# 現在のテンプレート全体をJOIの不正審査に通過させておきたい(一応)ので、現在のテンプレート全体(ALL版)を使用して提出します。
# これは一次予選第一回A問題のみで、他の問題は最低限のテンプレートに絞った SHORT版 で提出します。
# 基本的には、SHORT版にコーディングをし、必要なテンプレートをALL版からコピペするという方針になります。
# また、考察の過程や問題にあったコード改変の為に、(必要ない/改造された)テンプレコードがSHORT版提出コードに混じる可能性があります。
# SHORT版はこちら: https://github.com/Rino-program/atcoder/blob/main/.template/main.py
# 現在(26/09/12)ローカルにある最新のテンプレコードを事前にgithubにプッシュしてあります。(コンテスト開催前)
# ※このテンプレートコードの内容は今後変更される可能性があります。また、現在MIT licenseで公開されています。
# ※このテンプレートには名残や削除をさぼっているせいで全く意味のないコード(gcdなど)があります。
