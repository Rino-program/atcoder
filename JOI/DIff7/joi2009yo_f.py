...
def main():
    ...
    K = N * N
    MOD_VAL = 100000
    # dp[j][k] : j個選んで総和がkになる組み合わせの数
    dp = [[0] * (S + 1) for _ in range(K + 1)]
    dp[0][0] = 1
    # 1からMまでの整数を順に考慮
    # ※ Sより大きい整数を選んでも総和がSを超えるだけなので、上限は min(M, S) で十分です
    for i in range(1, min(M, S) + 1):
        # 同じ整数を重複して選ばないよう、個数・総和ともに後ろからループを回す
        for j in range(K, 0, -1):
            for k in range(i, S + 1):
                dp[j][k] = (dp[j][k] + dp[j - 1][k - i]) % MOD_VAL
    # K(N^2)個選んで総和がSになる組み合わせの数を出力
    print(dp[K][S])
