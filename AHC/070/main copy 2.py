...
def main() -> None:
    ...
    for i in range(N ** 2):
        ...
        n2 = (nA2, nB2)
        n3 = (nA3, nB3)
        ne = max((0 if Map[n1[0] * N + n1[1]] == 1 else 1, MapAB[n1[0] * N + n1[1]], abs(A - n1[0]) + abs(B - n1[1])), (0 if Map[n2[0] * N + n2[1]] == 1 else 1, MapAB[n2[0] * N + n2[1]], abs(A - n2[0]) + abs(B - n2[1])), (0 if Map[n3[0] * N + n3[1]] == 1 else 1, MapAB[n3[0] * N + n3[1]], abs(A - n3[0]) + abs(B - n3[1])))
        ans_now = None
        ne = ne[2]
        if ne == abs(A - n1[0]) + abs(B - n1[1]):
            now = n1
            ans_now = 0
        elif ne == abs(A - n2[0]) + abs(B - n2[1]):
            now = n2
            ans_now = 1
        else:
            now = n3
