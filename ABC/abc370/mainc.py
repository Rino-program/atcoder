...
def main() -> None:
    ...
    T = STR()
    li = []
    for i, s, t in zip(range(len(S)), S, T):
        if s == t: continue
        li.append(tuple([LOWER.index(S[j]) for j in range(i)] + [LOWER.index(t)] + [LOWER.index(S[j]) for j in range(i+1, len(S))]))
        debug(li[-1])
    li.sort()
