...
def main() -> None:
    ...
    N, M = MAP()
    P = LIST()
    P = [(p, i) for i, p in en(P)]
    minseg = SegTree(P, op=min, e=(INF, -1))
    maxseg = SegTree(P, op=max, e=(-INF, -1))
    
    for _ in range(M):
        L, R = MAP()
        tmp = (minseg.query(L-1, R), maxseg.query(L-1, R))
        # print(tmp)
        minseg.set(tmp[0][1], (tmp[1][0], tmp[0][1]))
        minseg.set(tmp[1][1], (tmp[0][0], tmp[1][1]))
        maxseg.set(tmp[0][1], (tmp[1][0], tmp[0][1]))
        maxseg.set(tmp[1][1], (tmp[0][0], tmp[1][1]))
        """# debug
        ans = []
        for i in range(N):
            ans.append(minseg.get(i)[0])
        print(*ans)"""
    ans = []
    for i in range(N):
        ans.append(minseg.get(i)[0])
