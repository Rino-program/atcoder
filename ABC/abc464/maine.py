...
def main() -> None:
    # ここに解答を書く
    H, W, Q = MAP()
    RCX = [input().split() for i in range(Q)]
    RCX = list(map(list, list(set(map(tuple, RCX)))))
    for i in range(Q):
        RCX[i][0] = int(RCX[i][0])
        RCX[i][1] = int(RCX[i][1])
    CX = []
    idx_di = dedict(int)
    for i, (R, C, X) in en(RCX):
        R, C = R - 1, C - 1
        idx_di[(R,C,X)] = i
        if R != H-1: continue
        while CX and CX[-1][0] <= C:
            CX.pop()
        CX.append((C, X, R))
    CX.reverse()
    debug(CX)
    ...
    for i in range(H-1, -1, -1):
        ...
        for j in range(W-1, -1, -1):
            ...
            ans[i][j] = now
        if i-1 in RCX_di:
            for C, X, R in RCX_di[i-1]:
                while CX and CX[-1][0] <= C and idx_di[(CX[-1][2], CX[-1][0], CX[-1][1])] < idx_di[(R, C, X)]:
                    CX.pop()
                CX.append((C, X, R))
    print_grid(ans)
