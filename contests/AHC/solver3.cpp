...
int main() {
    ...
    for (int i = 0; i < N; i++)
        for (int j = 0; j < N; j++) {
            cin >> Map[i][j];
            if (Map[i][j] != 0) {
                color_cells[Map[i][j]].push_back(cell_id(i, j));
                alive_color_count[Map[i][j]]++;
            }
        }
    ...
    while (length_snake < M && !hard_time_up()) {
        ...
        double reserve = max(OUTPUT_RESERVE * 0.6, min(0.022, 0.003 + 0.000012 * (double)turns_left));
        double budget_per_turn = (remaining - reserve) / turns_left;
        budget_per_turn = max(budget_per_turn, 0.01);
        budget_per_turn = min(budget_per_turn * budget_bias, 0.31);

        // 近くに必要色があるターンは探索予算を増やす
        if (has_immediate_target(hebi, length_snake)) {
            budget_per_turn = min(0.34, budget_per_turn * 1.30);
        }
        ...
        int max_extra_try = 2;
        if      (left0 >= 0.070) max_extra_try = 6;
        else if (left0 >= 0.045) max_extra_try = 5;
        else if (left0 >= 0.025) max_extra_try = 4;
        else if (left0 >= 0.014) max_extra_try = 3;

        int extra_try = 0;
        while (!time_up() && extra_try < max_extra_try) {
            double left = search_time_left_sec();
            if (left < 0.0062) break;
            if (extra_try >= 2 && left < 0.0105) break;
