...
def main() -> None:
    ...
    H, W, N = MAP()
    S = CHARSL(H)
    box = sum([1 if S[i][j] == '#' else 0 for i in range(H) for j in range(W)])
    T = CHARS()
    now = (0, 0)
    if S[now[0]][now[1]] == '#':
        box -= 1
    ...
    for i in T:
        if i == 'R':
            now = (now[0], min(now[1] + 1, W - 1))
        elif i == 'L':
            now = (now[0], max(now[1] - 1, 0))
        elif i == 'D':
            now = (min(now[0] + 1, H - 1), now[1])
        elif i == 'U':
            now = (max(now[0] - 1, 0), now[1])
        if S[now[0]][now[1]] == '#':
            box -= 1
            S[now[0]][now[1]] = '.'
    print(box)
