...
def build_weighted_graph(n: int, edges: list[tuple[int, int, int]], idx: int = True, directed: bool = False) -> list[list[int]]:
    """概要:
...
def main() -> None:
    ...
    P = LIST()
    UVW = TUPLES(M)
    g = build_weighted_graph(N, UVW, directed=True)
    P = [p + sum(g[i]) for i, p in enumerate(P)]
