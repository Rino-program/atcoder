...
def main() -> None:
    # ここに解答を書く
    N, Q = MAP()
    hato_idx = [i - 1 for i in range(N + 1)]
    su_idx = [i for i in range(1, N+1)]
    hako = [i for i in range(N)]
    for i in range(Q):
        inp = LIST()
        if inp[0] == 1:
            hato_idx[inp[1]] = hako[inp[2] - 1]
        elif inp[0] == 2:
            su_idx[hako[inp[1]-1]], su_idx[hako[inp[2]-1]] = su_idx[hako[inp[2]-1]], su_idx[hako[inp[1]-1]]
            hako[inp[1]-1] , hako[inp[2]-1] = hako[inp[2]-1], hako[inp[1]-1]
        else:
            print(su_idx[hato_idx[inp[1]]])
        debug(f"===== {str(i).zfill(2)} =====")
        debug(hato_idx)
        debug(su_idx)
        debug(hako)
